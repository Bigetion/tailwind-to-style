...
export class BundleAnalyzer {
  ...
  generateRecommendations() {
    ...
    if (categories.animations && categories.animations.size > 5000) {
      recommendations.push({
        ...
        suggestion:
          "Use CSS animations only where needed, consider removing unused keyframes",
        priority: "low",
      });
    }
  ...
  printReport() {
    ...
    console.log(
      `🗜️  Gzip Size: ${this.formatBytes(this.stats.gzipSize)} (estimated)`
    );

    console.log("\n📋 Categories:");
    const sortedCategories = Object.entries(this.stats.categories).sort(
    ...
    sortedCategories.forEach(([category, data]) => {
      ...
      console.log(
        `  ${category.padEnd(15)} ${this.formatBytes(data.size).padEnd(10)} (${percentage}%) - ${data.count} classes`
      );
    });
    ...
    if (this.stats.recommendations.length > 0) {
      ...
      this.stats.recommendations.forEach((rec, idx) => {
        ...
        console.log(
          `  ${idx + 1}. ${icon} [${rec.priority.toUpperCase()}] ${rec.message}`
        );
