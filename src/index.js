...
import {
  ...
  collectSSRCSS,
  createSSRCollector,
  // Legacy SSR (deprecated but kept for backward compatibility)
  startSSR,
  stopSSR,
  getSSRStyles,
} from "./utils/ssr.js";
...
export { createSSRCollector };

// Legacy SSR API (deprecated - kept for backward compatibility)
/**
 * @deprecated Use createSSRCollector() instead
 */
export { startSSR, stopSSR, getSSRStyles };
...
export function twsx(obj, options = {}) {
  ...
  if (_twsxInputCache.has(cacheKey)) {
    ...
    if (inject && (IS_BROWSER || isSSRCollecting())) {
...
// SSR Exports
// ============================================================================
// startSSR/stopSSR/getSSRStyles/createSSRCollector are exported at the top of
// this file, re-exporting the single implementation in ./utils/ssr.js.
