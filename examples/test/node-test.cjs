...
const supportsColor = process.stdout.isTTY && 
  (process.platform !== 'win32' || process.env.TERM !== 'dumb' || process.env.FORCE_COLOR);
...
function colorizeCSS(css) {
  return css
    ...
    .replace(/^([.#][^\s{]+)|\n([.#][^\s{]+)/g, (match, g1, g2) => {
      ...
      return match.replace(selector, `${colors.magenta}${selector}${colors.reset}`);
    });
}
...
console.log(`${colors.yellow}Note: For best formatting, use a terminal with width of at least 100 characters${colors.reset}`);
...
const beautifiedBasic = basicOutput.split(';').filter(p => p.trim()).map(prop => {
  const colonIndex = prop.indexOf(':');
  if (colonIndex > -1) {
    const name = prop.substring(0, colonIndex).trim();
    const value = prop.substring(colonIndex + 1).trim();
